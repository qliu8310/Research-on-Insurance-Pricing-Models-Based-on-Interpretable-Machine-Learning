...
for run_index in range(15):
    ...
    # ----------------------
    y_pred["train"][f"CAFTT"] = np.array([x for [x] in FT_transformer.predict(learn_data,batch_size=100000
                                                                                )["output"]])
    y_pred["test"][f"CAFTT"] = np.array([x for [x] in FT_transformer.predict(test_data,batch_size=100000
                                                                                )["output"]])
