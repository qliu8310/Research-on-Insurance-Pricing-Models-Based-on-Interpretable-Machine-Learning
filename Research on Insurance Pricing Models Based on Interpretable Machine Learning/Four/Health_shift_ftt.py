...
from keras.layers import Input, Dense, Concatenate, Dropout, BatchNormalization, Activation, Add
...
# 定义数值特征的输入维度
num_input_shape = num_train.shape[1]
cat_input_shape = cat_train.shape[1]

num_input_shape,cat_input_shape

# # 定义Transformer块
# class TransformerBlock(tf.keras.layers.Layer):
#     def __init__(self, emb_dim, num_heads, ff_dim, dropout_rate=0.1):   #emb_dim在这里表示特征wei'du
#         super(TransformerBlock, self).__init__()
#         self.dense_proj = tf.keras.layers.Dense(emb_dim)  # 新增的 Dense 层
#         self.reshape = tf.keras.layers.Reshape((-1, emb_dim))
#         self.att = tf.keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=emb_dim)
#         self.ffn = tf.keras.Sequential(
#             [tf.keras.layers.Dense(ff_dim, activation='relu'), tf.keras.layers.Dense(emb_dim)]
#         )
...
#         proj_input = self.dense_proj(inputs)  # 转换输入形状
#         reshaped_input = self.reshape(proj_input)  # 重塑输入形状
#         attn_output = self.att(reshaped_input, reshaped_input)   #使用多头注意力层处理输入
#         attn_output = self.dropout1(attn_output, training=training)  #对注意力层的输出应用 Dropout
#         out1 = self.layernorm1(proj_input + attn_output)  #层归一化，残差连接
#         ffn_output = self.ffn(out1)  #通过前馈网络
#         ffn_output = self.dropout2(ffn_output, training=training)  #对前馈网络的输出应用 Dropout
#         return self.layernorm2(out1 + ffn_output) 

# # 构建FT-Transformer模型
# class FTTransformer(tf.keras.Model):
#     def __init__(self, input_shape, emb_dim=32, num_transformer_blocks=3, num_heads=8, dropout_rate=0.2):
#         super(FTTransformer, self).__init__()
        
#         # 输入层
#         #self.input_layer = tf.keras.layers.InputLayer(input_shape=input_shape)   ###########修改过
#         self.input_layer = tf.keras.layers.InputLayer(input_shape=(input_shape,))
#         # Transformer块
#         self.transformer_blocks = [
#             TransformerBlock(emb_dim, num_heads, emb_dim*2, dropout_rate)
#             for _ in range(num_transformer_blocks)
#         ]
        
#         # 输出层
#         self.output_layer = tf.keras.layers.Dense(1, activation='exponential')
...
# 定义Transformer块
class TransformerBlock(tf.keras.layers.Layer):
    def __init__(self, input_dim, emb_dim, num_heads, ff_dim, dropout_rate=0.1):
        super(TransformerBlock, self).__init__()
        self.dense_proj = tf.keras.layers.Dense(emb_dim)
        self.emb_dim = emb_dim
        
#         # 核心修改：校验维度可整除
#         if input_dim % emb_dim != 0:
#             raise ValueError(f"input_dim={input_dim} 必须能被 emb_dim={emb_dim} 整除")
    ...
    def call(self, inputs, training):
        proj_input = self.dense_proj(inputs)
        reshaped_input = self.reshape(proj_input)
        attn_output = self.att(reshaped_input, reshaped_input)
        attn_output = self.dropout1(attn_output, training=training)
        out1 = self.layernorm1(proj_input + attn_output)
        ffn_output = self.ffn(out1)
        ffn_output = self.dropout2(ffn_output, training=training)
        return self.layernorm2(out1 + ffn_output)

# 构建FT-Transformer模型（移除InputLayer）
class FTTransformer(tf.keras.Model):
    def __init__(self, input_dim, emb_dim=12, num_transformer_blocks=3, num_heads=8, dropout_rate=0.2):
        super(FTTransformer, self).__init__()
        
        # 传递 input_dim 给每个 TransformerBlock
        self.transformer_blocks = [
            TransformerBlock(input_dim, emb_dim, num_heads, emb_dim*2, dropout_rate)
            for _ in range(num_transformer_blocks)
        ]
...
# 实例化模型
input_shape = num_input_shape + cat_input_shape
#inputs = tf.keras.Input(shape=(input_shape,))
model = FTTransformer(input_shape)
...
# 模型训练
history = model.fit(np.concatenate((num_train, cat_train), axis=1), y_train,
...
y_pred=model.predict(np.concatenate((num_test, cat_test), axis=1))
print(y_pred.shape)


# 将 RaggedTensor 转换为 Tensor
y_pred= tf.reduce_mean(y_pred, axis=1)  # 求平均值来减少不规则维度

# 确认转换后的形状
print("y_pred_dense shape:", y_pred.shape)
...
# tf.reduce_sum(deviance_per_obs*sample_weight)
...
exposure_weighted_average,deviance_per_obs

# 拟合值
y_fitted = model.predict(np.concatenate((num_train, cat_train), axis=1))

y_fitted= tf.reduce_mean(y_fitted, axis=1)  # 求平均值来减少不规则维度
...
predictions_1_3['transformer_predictions'] = test_reset_index["predictions"]

fitted_1_3['transformer_fitted']=train["fitted"]
fitted_predictions_1_3['transformer_fitted_predictions']=df['predictions']
...
# 分割数值特征和分类特征（用于构建模型）  
num_train_1_2 = X_train_preprocessed_1_2[:, :len(numeric_cols)]  
cat_train_1_2 = X_train_preprocessed_1_2[:, len(numeric_cols):]  
num_test_1_2 = X_test_preprocessed_1_2[:, :len(numeric_cols)]  
cat_test_1_2 = X_test_preprocessed_1_2[:, len(numeric_cols):]  
...
# 定义数值特征的输入维度  
num_input_shape_1_2 = num_train_1_2.shape[1]  
cat_input_shape_1_2=cat_train_1_2.shape[1]
num_input_shape_1_2,cat_input_shape_1_2


# input_shape_1_2 = num_input_shape_1_2 + cat_input_shape_1_2

# # 构建模型
# model_1_2 = FTTransformer(input_shape_1_2,emb_dim=12)
# model_1_2.build(input_shape=(None, input_shape_1_2))

# # 调用模型初始化权重
# dummy_input = tf.random.normal((1, input_shape_1_2))
# _ = model_1_2(dummy_input)

# # 编译模型（必须在加载权重前编译，确保层结构稳定）
# model_1_2.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4), loss='poisson')

# # 加载纯模型权重
# model_1_2.load_weights('model.weights.h5')

# # 模型训练
# history_1_2 = model_1_2.fit(np.concatenate((num_train_1_2, cat_train_1_2), axis=1), y_train_1_2,
...
history_1_2 = model_1_2.fit(
    np.concatenate((num_train_1_2, cat_train_1_2), axis=1),
    y_train_1_2,
    sample_weight=w_train_1_2,
    epochs=100,
    batch_size=32
...
y_pred_1_2=model_1_2.predict(np.concatenate((num_test_1_2, cat_test_1_2), axis=1))
# 将 RaggedTensor 转换为 Tensor
y_pred_1_2= tf.reduce_mean(y_pred_1_2, axis=1)  # 求平均值来减少不规则维度

# print(y_pred_1_2)
mse = mean_squared_error(y_test_1_2,y_pred_1_2.numpy())
print(f"Mean Squared Error: {mse}")
...
# 对训练集进行预测
y_fitted_1_2 = model_1_2.predict(np.concatenate((num_train_1_2, cat_train_1_2), axis=1))
y_fitted_1_2= tf.reduce_mean(y_fitted_1_2, axis=1)  # 求平均值来减少不规则维度
...
predictions_1_2['transformer_predictions'] = test_1_2_reset_index["predictions"]

fitted_1_2['transformer_fitted']=train_1_2["fitted"]
fitted_predictions_1_2['transformer_fitted_predictions']=df_1_2['predictions']
...
# 分割数值特征和分类特征（用于构建模型）  
num_train_2_3 = X_train_preprocessed_2_3[:, :len(numeric_cols)]  
cat_train_2_3 = X_train_preprocessed_2_3[:, len(numeric_cols):]  
num_test_2_3 = X_test_preprocessed_2_3[:, :len(numeric_cols)]  
cat_test_2_3 = X_test_preprocessed_2_3[:, len(numeric_cols):]  
...
# 定义数值特征的输入维度  
num_input_shape_2_3 = num_train_2_3.shape[1]  
cat_input_shape_2_3=cat_train_2_3.shape[1]
num_input_shape_2_3,cat_input_shape_2_3

input_shape_2_3 = num_input_shape_2_3 + cat_input_shape_2_3
inputs_2_3 = tf.keras.Input(shape=(input_shape_2_3,))
model_2_3 = FTTransformer(input_shape_2_3)
model_2_3.build(input_shape=(None, input_shape_2_3))
...
model_2_3.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
              loss='poisson')
model_2_3.load_weights('model.weights.h5')

# 模型训练
history_2_3= model_2_3.fit(np.concatenate((num_train_2_3, cat_train_2_3), axis=1), y_train_2_3,
...
y_pred_2_3=model_2_3.predict(np.concatenate((num_test_2_3, cat_test_2_3), axis=1))
# 将 RaggedTensor 转换为 Tensor
y_pred_2_3= tf.reduce_mean(y_pred_2_3, axis=1)  # 求平均值来减少不规则维度

# print(y_pred_2_3)
test_2_3["predictions"]=y_pred_2_3
mse = mean_squared_error(y_test_2_3,y_pred_2_3.numpy())
print(f"Mean Squared Error: {mse}")
...
#拟合图
y_fitted_2_3 = model_2_3.predict(np.concatenate((num_train_2_3, cat_train_2_3), axis=1))
y_fitted_2_3= tf.reduce_mean(y_fitted_2_3, axis=1)  # 求平均值来减少不规则维度
...
predictions_2_3['transformer_predictions'] = test_2_3_reset_index["predictions"]

fitted_2_3['transformer_fitted']=train_2_3["fitted"]
fitted_predictions_2_3['transformer_fitted_predictions']=df_2_3['predictions']
...
# 分割数值特征和分类特征（用于构建模型）  
num_train_2_1 = X_train_preprocessed_2_1[:, :len(numeric_cols)]  
cat_train_2_1 = X_train_preprocessed_2_1[:, len(numeric_cols):]  
num_test_2_1 = X_test_preprocessed_2_1[:, :len(numeric_cols)]  
cat_test_2_1 = X_test_preprocessed_2_1[:, len(numeric_cols):]  
...
# 定义数值特征的输入维度  
num_input_shape_2_1 = num_train_2_1.shape[1]  
cat_input_shape_2_1=cat_train_2_1.shape[1]

input_shape_2_1 = num_input_shape_2_1 + cat_input_shape_2_1
inputs_2_1 = tf.keras.Input(shape=(input_shape_2_1,))
model_2_1 = FTTransformer(input_shape_2_1)

model_2_1.build(input_shape=(None, input_shape_2_1))
...
# 模型训练
history_2_1= model_2_1.fit(np.concatenate((num_train_2_1, cat_train_2_1), axis=1), y_train_2_1,
...
y_pred_2_1=model_2_1.predict(np.concatenate((num_test_2_1, cat_test_2_1), axis=1))
# 将 RaggedTensor 转换为 Tensor
y_pred_2_1= tf.reduce_mean(y_pred_2_1, axis=1)  # 求平均值来减少不规则维度

# print(y_pred_2_1)

test_2_1["predictions"]=y_pred_2_1

mse = mean_squared_error(y_test_2_1,y_pred_2_1.numpy())
print(f"Mean Squared Error: {mse}")
...
# 拟合图
y_fitted_2_1 = model_2_1.predict(np.concatenate((num_train_2_1, cat_train_2_1), axis=1))
y_fitted_2_1= tf.reduce_mean(y_fitted_2_1, axis=1)  # 求平均值来减少不规则维度
...
predictions_2_1['transformer_predictions'] = test_2_1_reset_index["predictions"]

fitted_2_1['transformer_fitted']=train_2_1["fitted"]
fitted_predictions_2_1['transformer_fitted_predictions']=df_2_1['predictions']
